...
| Berm Height (`berm_height`) | float | [0, 100000] | 2 | m | Height of the berm crest above sea level, in metres: the top of the beach face, where the gentle foreshore ends and the steep backing slope begins. Raising it lifts the crest and so widens the beach face for a given Angle; it is the "how much beach" knob. | no |
